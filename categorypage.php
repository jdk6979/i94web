...
$pageSize = 6;
if($totalArr =  fetchArray("select count(post_category) as totalCount from posts where post_category = $id"))
{
     //总得记录数,数据表提供
...
$resPostsSQL  = 
	...
	and
	             posts.post_category = $id
        	order by 
                	posts.post_date desc 
        	limit " .($currentPage-1)*$pageSize.",$pageSize";
// 获取类别名字
$categoryName = fetchArray("select category_name from category where category_id = $id");
?>
<!DOCTYPE html>
<html>
	<head>
						...
						<ul>
							<?php
								if($currentPage != 1)
					                          	{
					                          ?>
					                          		<li><a href="categorypage.php?id=<?php echo $id; ?>&amp;num=<?php echo ($currentPage-1);?>">上一页</a></li>
					                          <?php
					                      		}
								for($pageI = 1; $pageI <= $pageCount; $pageI++)
								{
					                          ...
					                          ?>  
					                          	...
					                          	<?php 
						                      		...
						                      		{
					                          	?>
					                          			<li><a href="categorypage.php?id=<?php echo $id; ?>&amp;num=<?php echo $pageI;?>"><?php echo $pageI;?></a></li>
					                          <?php
						                          	}
						                          }
					                          	// 下一页
					                          	...
					                          	{
					                          ?>
					                          		<li><a href="categorypage.php?id=<?php echo $id; ?>&amp;num=<?php echo ($currentPage+1);?>">下一页</a></li>
					                          <?php
					                          	}
					                       	?>
						</ul>
